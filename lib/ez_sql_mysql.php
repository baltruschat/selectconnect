		...
		*/
		...
		function connect($dbuser='', $dbpassword='', $dbhost='localhost')
		{
			...
			if ( ! $dbuser )
			{
				...
				$this->show_errors ? trigger_error($ezsql_mysql_str[1],E_USER_ERROR) : null;
			}
			...
			else if ( ! $this->dbh = @mysql_connect($dbhost,$dbuser,$dbpassword,true,131074) )
			{
				...
				$this->show_errors ? trigger_error($ezsql_mysql_str[2],E_USER_ERROR) : null;
			}
			else
			{
				$this->dbuser = $dbuser;
		...
		*/
		...
		function select($dbname='', $encoding='')
		{
			...
			if ( ! $dbname )
			{
				...
				$this->show_errors ? trigger_error($ezsql_mysql_str[3],E_USER_ERROR) : null;
			}
			...
			else if ( ! $this->dbh )
			{
				...
				$this->show_errors ? trigger_error($ezsql_mysql_str[4],E_USER_ERROR) : null;
			}
			...
			else if ( !@mysql_select_db($dbname,$this->dbh) )
			{
				...
				$this->show_errors ? trigger_error($str,E_USER_ERROR) : null;
			}
			else
			{
				$this->dbname = $dbname;
